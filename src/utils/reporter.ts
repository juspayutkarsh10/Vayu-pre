...
import { PortalResult, OverallSummary } from "../types";
...
export function generateReport(
  ...
  const portalTabs = portalResults
    .map(
      (pr, idx) => `
  ...
  let globalIndex = 0;
  const portalContents = portalResults
    .map((pr, portalIdx) => {
      const { results, summary, portal } = pr;
      const flakyCount = results.filter(
        (r) => r.retries && r.retries.length > 0 && r.status === "pass"
      ).length;
    ...
    <div class="portal-content ${portalIdx === 0 ? "active" : ""}" id="portal-${portal.id}">
      ...
      <div class="stored-values">
        ...
        <div class="values-grid">
          ${Object.entries(summary.storedValues || {})
            .map(
              ([key, value]) => `
